...
from nuscenes.eval.detection.algo_fusa import accumulate_weighted
...
class DetectionEval:
    ...
    def __init__(self,
                 ...
                 output_dir: str = None,
                 verbose: bool = True,
                 fusa_weighting_config: str = None):
        """
        ...
        self.cfg = config
        self._fusa_spec = (
            load_fusa_weighting_spec(fusa_weighting_config) if fusa_weighting_config else None
        )
    ...
    def evaluate(self) -> Tuple[DetectionMetrics, DetectionMetricDataList]:
        ...
        if self._fusa_spec is not None and self.verbose:
            print('FUSA weighting enabled (per-GT + class aggregation).')
        metric_data_list = DetectionMetricDataList()
        for class_name in self.cfg.class_names:
            for dist_th in self.cfg.dist_ths:
                if self._fusa_spec is not None:
                    md = accumulate_weighted(
                        self.gt_boxes,
                        self.pred_boxes,
                        class_name,
                        self.cfg.dist_fcn_callable,
                        dist_th,
                        self._fusa_spec.gt_weight,
                    )
                else:
                    md = accumulate(
                        self.gt_boxes,
        ...
        if self._fusa_spec is not None:
            metrics = FusaDetectionMetrics(self.cfg, self._fusa_spec)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fusa_weighting', type=str, default='',
                        help='Optional JSON: per-class / ego-distance / attribute weights (FUSA).')
    ...
    fusa_w_ = args.fusa_weighting.strip() or None
