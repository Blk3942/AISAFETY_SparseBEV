...
SAFETY_GT_COLOR = (180, 0, 255)  # RGB, magenta-ish


def _load_safety_cfg(path: str):
...
def _filter_safety_gt_boxes(bboxes_gt, safety_max_dist: float, class_range: dict):
    """
    Safety-critical GT口径（近似实现）：
    - 在 LiDAR/ego 平面按 center 的 2D 距离过滤：dist < min(class_range[class], safety_max_dist)
    说明：safety_critical_eval 的 devkit 口径是用 (global - ego_pose translation) 得到 ego_dist；
    这里用 LiDAR/ego 坐标下的中心距做近似，足以用于可视化对比。
    """
    out = []
...
def viz_bbox_block_overlay(nusc, bboxes, data_info, fig, gs, row_start, block_title, rgb, linewidth=1.5):
    """
    在同一 block 的所有视图上叠加一层 bbox（用于 safety GT 之类的对照层）。
    注意：该函数不绘制背景图，只叠加框；调用顺序应在主 block 绘制之后。
    """
    cam_types = [
        'CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT',
        'CAM_BACK_RIGHT', 'CAM_BACK', 'CAM_BACK_LEFT',
    ]
    c_rgb = np.array(rgb) / 255.0
...
def main():
    ...
    parser.add_argument('--max_samples', type=int, default=16, help='Max val frames to dump (顺序前N个)')
    parser.add_argument('--random_samples', type=int, default=0, help='随机抽样数量（>0时覆盖 max_samples）')
    parser.add_argument('--seed', type=int, default=0, help='随机抽样seed')
    parser.add_argument(
        '--force_single_frame',
        action='store_true',
        help='强制按单帧(6相机)推理/可视化：将 sweeps_num=0，并将模型期望帧数降为1，避免T*N维度不匹配',
    )
    ...
    parser.add_argument(
        ...
        help='safety_critical_eval 配置（用于 safety GT 过滤可视化）',
    )
    parser.add_argument('--viz_safety_gt', action='store_true', help='在GT block上叠加 safety_critical_eval 口径GT')
    ...
    if args.force_single_frame:
        # 保持模型结构不变（便于严格加载 checkpoint），但允许在运行时将单帧输入复制成多帧视角。
        pass
    ...
    if args.random_samples and args.random_samples > 0:
        ...
        selected = sorted(random.sample(range(len(val_dataset)), k=min(args.random_samples, len(val_dataset))))
    else:
        selected = list(range(min(args.max_samples, len(val_dataset))))

    selected_set = set(selected)
    logging.info('Dumping %d samples to %s' % (len(selected), args.out_dir))

    if args.viz_safety_gt:
        safety_max_dist, class_range = _load_safety_cfg(args.safety_cfg)
        logging.info('Safety GT enabled: safety_max_dist=%.1f, cfg=%s' % (safety_max_dist, args.safety_cfg))
    else:
    ...
    for i, data in enumerate(val_loader):
        ...
        with torch.no_grad():
            if args.force_single_frame:
                # 让输入的 img 视角数与 img_metas 里记录的视角数一致（避免 T*N 维度不匹配）。
                try:
                    metas = data['img_metas'].data[0]
                    meta0 = metas[0]
                    target_views = len(meta0.get('filename', []))
                    img_dc = data['img']
                    img_tensor = img_dc.data[0]
                    if img_tensor.ndim == 5 and target_views and img_tensor.shape[1] != target_views:
                        rep = int(np.ceil(target_views / img_tensor.shape[1]))
                        img_tensor = img_tensor.repeat(1, rep, 1, 1, 1)[:, :target_views]
                        img_dc.data[0] = img_tensor

                    # 同步扩展一些常见的 list 型 meta 字段（如果是单帧则复制）。
                    for k in ['filename', 'img_timestamp']:
                        v = meta0.get(k, None)
                        if isinstance(v, list) and target_views and len(v) != target_views:
                            rep = int(np.ceil(target_views / len(v)))
                            meta0[k] = (v * rep)[:target_views]

                    # lidar2img 可能是 list / np.ndarray / torch.Tensor
                    v = meta0.get('lidar2img', None)
                    if target_views and v is not None:
                        if isinstance(v, list) and len(v) != target_views:
                            rep = int(np.ceil(target_views / len(v)))
                            meta0['lidar2img'] = (v * rep)[:target_views]
                        elif isinstance(v, np.ndarray) and v.ndim >= 3 and v.shape[0] != target_views:
                            rep = int(np.ceil(target_views / v.shape[0]))
                            meta0['lidar2img'] = np.concatenate([v] * rep, axis=0)[:target_views]
                        elif torch.is_tensor(v) and v.ndim >= 3 and v.shape[0] != target_views:
                            rep = int(np.ceil(target_views / v.shape[0]))
                            meta0['lidar2img'] = v.repeat((rep,) + (1,) * (v.ndim - 1))[:target_views]
                except Exception:
                    # 兜底：不让可视化因为 meta 处理失败而直接中断
                    pass
        ...
        if args.viz_safety_gt and safety_max_dist is not None:
            bboxes_safety_gt = _filter_safety_gt_boxes(bboxes_gt, safety_max_dist, class_range or {})
            viz_bbox_block_overlay(
                nusc,
                bboxes_safety_gt,
                info,
                fig,
                gs,
                row_start=0,
                block_title='Safety-critical GT',
                rgb=SAFETY_GT_COLOR,
                linewidth=2.2,
            )
