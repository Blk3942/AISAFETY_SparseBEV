# offline_nuscenes_eval：nuScenes 检测评测逻辑说明

本文说明本目录内 **nuScenes 3D 目标检测** 离线评测的完整数据流、匹配规则、指标定义及与上游 devkit 的对应关系。算法实现主要来自 [nuScenes devkit](https://github.com/nutonomy/nuscenes-devkit)（Apache-2.0），与 MMDetection3D 中 `NuScenesDataset._evaluate_single()` 所调用的逻辑一致。
...
项目根目录下 **`tools/official_nuscenes_eval.py`** 会将本目录加入 `sys.path` 首位，从而 **无需 `pip install nuscenes`** 即可 `import nuscenes` 并跑完整评测。
...
`tools/official_nuscenes_eval.py` 中的 **`nds_from_summary()`** 用同一公式对 `metrics_summary.json` 做自检，应与 `nd_score` 一致。
...
- **推荐**：`render_curves=False`、`plot_examples=0`（与 `tools/official_nuscenes_eval.py` 默认一致）→ 仅需 **numpy、pyquaternion、tqdm**。  
...
python tools/official_nuscenes_eval.py /path/to/submission/results_nusc.json \
...
**FUSA 定制化**：使用独立目录 **`fusa_eval/`**（由本目录复制）+ 入口 **`tools/fusa_eval.py`**（仅把 `fusa_eval/` 加入 `sys.path`，不依赖 `offline_nuscenes_eval/`）。标准 NDS 仍用 **`tools/official_nuscenes_eval.py`** + **`offline_nuscenes_eval/`**。
